...
object LoadoutConfig : CategoryKt("Loadout Keybinds") {
    val enabled by boolean(false) {
        name = TranslatableValue.literal("Enabled")
    }

    val showEquipped by boolean(true) {
        name = TranslatableValue.literal("Show Equipped")
        description = TranslatableValue.literal("Show the currently equipped loadout in the menu")
    }

    val preventUnequip by boolean(true) {
        name = TranslatableValue.literal("Prevent Unequip")
        description = TranslatableValue.literal("Prevents unequipping your current loadout")
    }

    val autoClose by boolean(true) {
        name = TranslatableValue.literal("Auto Close")
        description = TranslatableValue.literal("Automatically close the loadout menu after swapping")
    }
